...
from app.models.text_recognition import TextRecognitionAnswerResponse, TextRecognitionMetadata, TextRecognitionErrorResponse
...
def create_text_recognition_router(settings: Settings) -> APIRouter:
    ...
    async def get_text_recognition_metrics(
        ...
        return {
            ...
            "performance": {
                ...
                "p95_processing_time_ms": round(performance_stats.p95_processing_time_ms, 2),
    ...
    async def text_recognition_health_check() -> JSONResponse:
        ...
        try:
            ...
            # 최근 성능 통계
            recent_stats = metrics_collector.get_performance_stats(time_range_minutes=10)
            ...
            health_data = {
                ...
                "cache": {
                    "enabled": "l1_memory" in cache_stats,  # Memory cache is always enabled if present
                    "hit_rate": cache_stats["total"]["hit_rate"] if "total" in cache_stats else 0,
                    "memory_usage_mb": cache_stats["l1_memory"]["memory_usage_mb"] if "l1_memory" in cache_stats else 0
                },
                "performance": {
                    "recent_requests": recent_stats.total_requests,
                    "success_rate": round(recent_stats.success_rate, 3),
                    "avg_processing_time_ms": round(recent_stats.avg_processing_time_ms, 2)
                } if recent_stats.total_requests > 0 else {}
            }
