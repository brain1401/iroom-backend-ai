...
class MetricsCollector:
    ...
    def get_performance_stats(
        self, time_range_minutes: int | None = None
    ) -> PerformanceStats:
        """
        성능 통계 조회 (캐싱 지원)
        ...
        stats = PerformanceStats(
            ...
            avg_confidence=(
                sum(m.confidence_avg for m in successful_metrics)
    ...
    def get_error_analysis(self, time_range_minutes: int = 60) -> dict[str, Any]:
        ...
        failed_metrics = [
            m
            for m in self.text_recognition_metrics
        ...
        # 전체 메트릭 중 오류 비율
        total_metrics = len(
            [m for m in self.text_recognition_metrics if m.timestamp >= cutoff_time]
        )
    ...
    def get_quality_metrics(self, time_range_minutes: int = 60) -> dict[str, Any]:
        ...
        # 전반적 품질 통계
        avg_confidence = sum(m.confidence_avg for m in recent_metrics) / len(
            recent_metrics
        )
        avg_questions_detected = sum(
            m.questions_detected for m in recent_metrics
        ) / len(recent_metrics)
    ...
    async def _check_alerts(self):
        ...
        try:
            ...
            # 성공률 체크
            if (
                stats.total_requests >= 10  # 최소 요청 수
                and stats.success_rate < self.alert_thresholds["success_rate"]
            ):
    ...
    async def _send_alert(
        self, alert_type: str, level: AlertLevel, message: str, context: dict[str, Any]
    ):
        """
        알림 전송 (쿨다운 적용)
        ...
        # 쿨다운 체크
        if (
            alert_type in self._last_alerts
            and current_time - self._last_alerts[alert_type] < self._alert_cooldown
        ):
            return
    ...
    def get_dashboard_data(self) -> dict[str, Any]:
        ...
        latest_system_metric = (
            list(self.system_metrics)[-1] if self.system_metrics else None
        )
        ...
        return {
            ...
            "performance": {
                ...
                "avg_processing_time_ms": round(
                    current_stats.avg_processing_time_ms, 2
                ),
                "p95_processing_time_ms": round(
                    current_stats.p95_processing_time_ms, 2
                ),
            ...
            "system": (
                {
                    "cpu_usage_percent": (
                        latest_system_metric.cpu_usage_percent
                        if latest_system_metric
                        else 0
                    ),
                    "memory_usage_mb": (
                        latest_system_metric.memory_usage_mb
                        if latest_system_metric
                        else 0
                    ),
                    "active_requests": (
                        latest_system_metric.active_requests
                        if latest_system_metric
                        else 0
                    ),
                    "queue_size": (
                        latest_system_metric.queue_size if latest_system_metric else 0
                    ),
                }
                if latest_system_metric
                else {}
            ...
            "health_status": self._determine_health_status(
                current_stats, error_analysis
            ),
        }

    def _determine_health_status(
        self, stats: PerformanceStats, error_analysis: dict[str, Any]
    ) -> str:
        """
        시스템 건강 상태 결정
