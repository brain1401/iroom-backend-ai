...
from app.models.text_recognition import TextRecognitionAnswerResponse, TextRecognitionAnswer, TextRecognitionMetadata, TextRecognitionErrorResponse
        ...
        async with self.semaphore:  # 동시성 제어
            async with self.rate_limiter:  # Rate limiting
                try:
                    ...
                    ocr_result = await self._call_gemini_vision(optimized_image, gemini_model)
...
"""
    ...
    async def process_batch(
        ...
        # 비동기 처리 태스크 생성
        tasks = [
            self._process_single_ocr(item, batch_id) 
        ...
        for completed_task in asyncio.as_completed(tasks):
            try:
                ...
                if progress.completed_items > 0:
                    estimated_total_time = elapsed_time * (len(items) / progress.completed_items)
                    remaining_time = estimated_total_time - elapsed_time
                    if remaining_time > 0:
                        progress.estimated_completion = datetime.fromtimestamp(datetime.now().timestamp() + remaining_time)
                ...
                # 태스크 처리 중 예외 발생
                logger.error("배치 처리 태스크 오류", batch_id=str(batch_id), error=str(e))
                progress.failed_items += 1
                yield progress
