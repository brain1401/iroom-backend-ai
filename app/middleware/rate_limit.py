...
class GeminiRateLimiter:
    ...
    def __init__(self, settings: Settings):
        ...
        self.user_tokens: dict[str, dict[str, int]] = defaultdict(
            lambda: {"daily": 0, "last_reset": 0}
        )
        self.circuit_breaker: dict[str, dict[str, object]] = defaultdict(
            lambda: {
                "failures": 0,
                "last_failure": 0.0,
    ...
    def _check_request_rate(self, user_key: str) -> bool:
        ...
        self.user_requests[user_key] = [
            req_time
            for req_time in self.user_requests[user_key]
    ...
    def _check_token_limit(self, user_key: str, estimated_tokens: int) -> bool:
        ...
        return (
            current_daily + estimated_tokens
        ) <= self.settings.rate_limit_tokens_per_day
    ...
    async def check_limits(self, request: Request, estimated_tokens: int = 0) -> bool:
        ...
        # Check token limit
        if estimated_tokens > 0 and not self._check_token_limit(
            user_key, estimated_tokens
        ):
            raise HTTPException(
                status_code=429,
